...
| `meshlog.py` | копия из корня, байт в байт |
...
- **Copy-кнопка генерируется в HTML на этапе сборки**, а не навешивается скриптом —
  поэтому «у каждого блока кода есть кнопка» проверяется статически.
- Блок-цитаты с эмодзи-маркером становятся callout-карточками:
  ⚠️ warning, 🚨 danger, 💡 tip, ✅ success.
...
`sw.js` кэширует хаб, оба гайда, `meshlog.py`, манифест и иконку. Флешер, прошивки и
